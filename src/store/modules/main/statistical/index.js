...
const { statisticalService } = allService
export default {
  // 1.启用命名空间
  namespaced: true,
  // 2.定义状态
  ...
  actions: {
    async data(context, payload) {
      ...
      const result = await statisticalService.getData(payload, config)
    ...
    async list(context, payload) {
      ...
      const result = await statisticalService.getList(payload, config)
    ...
    async detail(context, payload) {
      ...
      const result = await statisticalService.getDetail(payload, config)
 ...
 *
    ...
    // test statistical store
    // 1.获取store的数据
    this.$store.getters['statistical/data']
    // 2.分发一个action
    this.$store.dispatch('statistical/data') // 返回promise对象
    this.$store.getters['statistical/data']
    this.$store.getters['statistical/add'](100)
 */
